...
void main() {
    ...
    for (int i = 0; i < lights.length(); i++) {
        ...
        vec3 halfWayVec = normalize(lights[i].position + cameraPos);
        ...
        specular += material.specularReflection * pow(max(dot(normal, halfWayVec), 0.0), material.shininess) * lights[i].specularIntensity;
    }
